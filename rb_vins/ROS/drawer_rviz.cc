...
void DrawerRviz::publishOdometry() {
    ...
    // auto stamp      = ros::Time::now();
    double stamp_sec = latest_state_time_;
    ros::Time stamp = ros::Time().fromSec(stamp_sec);
...
void DrawerRviz::updateMap(const Eigen::Matrix4d &pose, double state_time) {
    std::unique_lock<std::mutex> lock(map_mutex_);
    pose_.R = pose.block<3, 3>(0, 0);
    pose_.t = pose.block<3, 1>(0, 3);
    latest_state_time_ = state_time;
    ismaprdy_ = true;
    update_sem_.notify_one();
}
