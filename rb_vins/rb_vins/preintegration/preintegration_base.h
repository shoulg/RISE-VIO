...
public:
    ...
    //drt_vio_init add
    Eigen::Matrix3d dq_dbg() const {
        return jacobian_.block<3, 3>(6, 9);
    }

    Eigen::Vector3d bg() const {
        return delta_state_.bg;
    }

    Eigen::Vector3d delta_p() const {
        return delta_state_.p;
    }

    Eigen::Quaterniond delta_q() const {
        return delta_state_.q;
    }

    Eigen::Vector3d delta_v() const {
        return delta_state_.v;
    }
...
public:
    ...
    // virtual deep copy interface
    virtual std::shared_ptr<PreintegrationBase> clone() const = 0;
