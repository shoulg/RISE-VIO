...
size_t MISC::getInsWindowIndex(const std::deque<std::pair<IMU, IntegrationState>> &window, double time) {
    ...
    // tmp change
    if (time == window.back().first.time) { 
        return window.size() - 1; // 或 return 0，视你下游逻辑而定 
    }
...
void MISC::insMechanization(const IntegrationConfiguration &config, const IMU &imu_pre, const IMU &imu_cur,
    ...
    // Vector3d dtheta = 0.5 * (imu_pre2.dtheta + imu_cur2.dtheta);
    ...
    if (config.iswithearth) {
        ...
        // Quaterniond q_pre = state.q;
        // state.q *= Rotation::rotvec2quaternion(dtheta);
        // state.q.normalize();

        // dvel = 0.5 * (q_pre.toRotationMatrix() * imu_pre2.dvel + state.q.toRotationMatrix() * imu_cur2.dvel) + config.gravity * dt;
...
void MISC::redoInsMechanization_new(const IntegrationConfiguration &config,
                                const IntegrationState &updated_state,
                                size_t reserved_ins_num,
                                std::deque<std::pair<IMU, IntegrationState>> &ins_windows) {
    // 可选：性能计时开始（仅用于测量，发布版可移除）
    // TimeCost tc;

    IntegrationState state = updated_state; // 累计使用的 state

    size_t index = getInsWindowIndex(ins_windows, state.time);
    if (index == 0 || index >= ins_windows.size()) {
        LOGE << "Failed to get right index in mechanization";
        return;
    }

    double dt = ins_windows.back().first.time - state.time;
    if (dt > 1.0) LOGW << "Redo INS mechanization long dt: " << dt << " s";

    // 处理 index 位置：尽量用 const 引用
    const IMU &imu_prev_ref = ins_windows[index - 1].first;
    const IMU &imu_cur_ref  = ins_windows[index].first;

    int isneed = isNeedInterpolation(imu_prev_ref, imu_cur_ref, state.time);
    if (isneed == -1) {
        // 直接在引用上计算（insMechanization 接口应为：insMechanization(config, const IMU&, const IMU&, IntegrationState&))
        insMechanization(config, imu_prev_ref, imu_cur_ref, state);
        ins_windows[index].second = state;
    } else if (isneed == 1) {
        state.time = imu_cur_ref.time;
        ins_windows[index].second = state;
    } else { // isneed == 2
        // 仅在需要插值时拷贝到临时变量，避免破坏主容器
        IMU tmp_prev = imu_prev_ref;
        IMU tmp_cur  = imu_cur_ref;
        imuInterpolation(tmp_cur, tmp_prev, tmp_cur, state.time);
        insMechanization(config, tmp_prev, tmp_cur, state);
        ins_windows[index].second = state;
    }

    // 顺序更新后续时间点，使用引用读取，避免多余拷贝
    const IMU *prev_ptr = &ins_windows[index].first; // 指向上一段的结束 IMU
    size_t n = ins_windows.size();
    for (size_t k = index + 1; k < n; ++k) {
        const IMU &cur_ref = ins_windows[k].first;
        insMechanization(config, *prev_ptr, cur_ref, state);
        ins_windows[k].second = state;
        prev_ptr = &cur_ref;
    }

    // 弹出过期历史（保持原行为）
    if (index < reserved_ins_num) return;
    size_t counts = index - reserved_ins_num;
    for (size_t i = 0; i < counts; ++i) ins_windows.pop_front();

    // 可选：打印耗时
    // LOGI << "redoInsMechanization cost " << tc.costInMillisecond() << " ms";
}
...
void MISC::gram_schmidt(const Eigen::Vector3d &gravity_inI, Eigen::Matrix3d &R_GtoI) {
