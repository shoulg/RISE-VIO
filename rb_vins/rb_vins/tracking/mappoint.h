...
public:
    ...
    static constexpr double DEFAULT_DEPTH  = 10.0;  //10.0 （5.0 室内 / 10.0 室外）10.0 2.0
    static constexpr double NEAREST_DEPTH  = 2.5;   // 最近可用路标点深度 （0.01 室内 / 1 室外）2.5 0.1
    static constexpr double FARTHEST_DEPTH = 200; // 最远可用路标点深度 （50 室内 / 200 室外）200 70
    ...
    void addObservation(double time, const Feature::Ptr &feature);
    ...
    Eigen::aligned_unordered_map<double, std::weak_ptr<Feature>> observations() {
        std::unique_lock<std::mutex> lock(mappoint_mutex_);
        return observations_;
    }
    ...
    void updatePoseAndDepth(const Vector3d& tmp_pose, const std::shared_ptr<Camera>& camera);

    size_t nextReferenceKid(ulong kid);

    bool updateReferenceFrame(const std::shared_ptr<Frame>& tmp_frame, const std::shared_ptr<Camera>& camera);

private:
    //drt_vio_init add
    // std::vector<std::weak_ptr<Feature>> observations_;
    Eigen::aligned_unordered_map<double, std::weak_ptr<Feature>> observations_;
