...
public:
    ...
    BiasSolverCostFunctor(const std::vector<Eigen::Vector3d> &bearings1,
                          ...
                          std::shared_ptr<PreintegrationBase> integratePtr) : _qic(qic) {

        jacobina_q_bg = integratePtr->dq_dbg();
        qjk_imu = integratePtr->delta_q();
    ...
    Create(const std::vector<Eigen::Vector3d> &bearings1,
           const std::vector<Eigen::Vector3d> &bearings2,
           const Eigen::Quaterniond &qic,
           std::shared_ptr<PreintegrationBase> integratePtr) {
...
struct WeightBiasSolverCostFunctor {
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    WeightBiasSolverCostFunctor(const std::vector<Eigen::Vector3d> &bearings1,
                          const std::vector<Eigen::Vector3d> &bearings2,
    ...
    Create(const std::vector<Eigen::Vector3d> &bearings1,
           ...
           const Eigen::Quaterniond &qic,
           std::shared_ptr<PreintegrationBase> integratePtr) {
        return (new ceres::AutoDiffCostFunction<WeightBiasSolverCostFunctor, 1, 3>(
                new WeightBiasSolverCostFunctor(bearings1, bearings2, w, qic, integratePtr)));
    }
