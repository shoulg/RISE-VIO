...
#ifndef REPROJECTION_FACTOR_W_H
#define REPROJECTION_FACTOR_W_H
...
class ReprojectionFactorW : public ceres::SizedCostFunction<2, 7, 7, 7, 1, 1> {

public:
    ReprojectionFactorW() = delete;

    // 标准差为归一化相机下的重投影误差观测, pixel / f
    ReprojectionFactorW(Vector3d pts0, Vector3d pts1, Vector3d vel0, Vector3d vel1, double td0, double td1, double std)
        : pts0_(std::move(pts0))
        , pts1_(std::move(pts1))
        , vel0_(std::move(vel0))
        , vel1_(std::move(vel1))
    ...
    bool Evaluate(const double *const *parameters, double *residuals, double **jacobians) const override {
        ...
        residual = w_ * sqrt_info_ * residual;
        ...
        if (jacobians) {
            ...
            if (jacobians[0]) {
                ...
                jacobian_pose_i.leftCols<6>() = w_ * reduce * jaco_i;
            ...
            if (jacobians[1]) {
                ...
                jacobian_pose_j.leftCols<6>() = w_ * reduce * jaco_j;
            ...
            if (jacobians[2]) {
                ...
                jacobian_ex_pose.leftCols<6>() = w_ * reduce * jaco_ex;
                jacobian_ex_pose.rightCols<1>().setZero();
            }

            if (jacobians[3]) {
                Eigen::Map<Eigen::Vector2d> jacobian_feature(jacobians[3]);
                jacobian_feature = -w_ * reduce * cbc * cnb1 * cb0n * cbc.transpose() * pts_0_td / (id0 * id0);
            }

            if (jacobians[4]) {
                Eigen::Map<Eigen::Vector2d> jacobian_td(jacobians[4]);
                jacobian_td = -w_ * reduce * cbc * cnb1 * cb0n * cbc.transpose() * vel0_ / id0 + sqrt_info_ * vel1_.head(2);
            }
        }
